...
const RegisterPage = () => {
  const [email, setEmail] = useState('frederik.noel@gmail.com')
  ...
  const checkMail = () => {
    ...
    if (!email) {
      setErrors({ ...errors, email: "L'e-mail doit être renseigné" })
      setDisabled(true)
    } else {
      if (!email.match(regexMail)) {
        setErrors({ ...errors, email: "L'e-mail est invalide" })
        setDisabled(true)
      } else {
  ...
  const checkConfirmPass = () => {
    if (!confirmPass) {
      ...
      if (!(confirmPass === password)) {
        setErrors({
          ...errors,
          confirmPass: 'Les mots de passe ne sont pas similaires',
        })
        setDisabled(true)
      } else {
